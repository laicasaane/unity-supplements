...
namespace System.Grid
{
    public partial struct SGridRange
    {
        public struct Enumerator : IEnumerator<SGridIndex>
        {
            ...
            public Enumerator(in SGridRange range)
                : this(range.Size, range.Clamped, range.Start, range.End, range.IsFromEnd, range.Direction)
            { }

            public Enumerator(in GridSize size, bool clamped, in SGridIndex start, in SGridIndex end, bool fromEnd, GridDirection direction)
            {
                this.offset = new GridIndex(
                    Offset(Math.Min(start.Row, end.Row)),
                    Offset(Math.Min(start.Column, end.Column))
                );

                var cStart = size.ClampIndex(start + this.offset);
                var cEnd = size.ClampIndex(end + this.offset);
            ...
            public Enumerator(GridDirection direction)
            {
                ...
                this.current = this.start = this.end = this.offset = default;
                this.startValue = this.endValue = default;
            ...
            public SGridIndex Current
            {
                get
                {
                    ...
                    return new SGridIndex(this.current.Row - this.offset.Row, this.current.Column - this.offset.Column);
                }
            }
